...
Fits the 127-joint MHR skeleton to SMPL output; saves per-frame joint positions and
bone rotation matrices for feet. The script (`tools/smpl_to_mhr_kpts.py`) runs from
this repo via pixi — no files need to be copied into the MHR directory.
...
Athlete_Mesh_Analysis_Codes/
  ...
  tools/
    ...
    smpl_to_mhr_kpts.py                <- Step 3: MHR skeleton fitting (runs via pixi)
